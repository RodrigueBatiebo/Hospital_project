...
<head>
    ...
    <title>@yield('title', 'Espace Secrétaire') — Clinique Santé+</title>
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .nav-item.active { background: #f0f7ff; color: #1a56a0; font-weight: 500; border-left-color: #1a56a0; }
        ...
        .avatar {
            ...
            font-size: 12px; font-weight: 600; color: #0c3870;
            flex-shrink: 0;
        }
        ...
        .btn-success { background: #d1e7dd; border-color: #0a3622; color: #0a3622; }
        ...
        .form-control:focus { outline: none; border-color: #1a56a0; box-shadow: 0 0 0 3px rgba(26,86,160,0.1); }
        ...
        /* ── Planning grid ───────────────────────────────── */
        .planning-grid { display: flex; flex-direction: column; gap: 8px; }
        .planning-slot {
            display: flex;
            justify-content: space-between;
            align-items: center;
        ...
        .slot-free   { background: #f5f5f5; color: #888; }
...
<aside class="sidebar">
    <div class="sidebar-header">
        ...
        <p>Service {{ auth()->user()->secretaire->service->nom_service ?? 'N/A' }}</p>
    </div>
    ...
    <nav class="nav-section">
        ...
        <a href="{{ route('secretaire.rendezvous') }}"
           class="nav-item {{ request()->routeIs('secretaire.rendezvous*') ? 'active' : '' }}">
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5">
            ...
            @if(isset($rdvEnAttente) && $rdvEnAttente > 0)
                <span class="badge badge-warn">{{ $rdvEnAttente }}</span>
            @endif
        </a>

        <a href="{{ route('medecin.planning') }}"
           class="nav-item {{ request()->routeIs('secretaire.planning') ? 'active' : '' }}">
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5">
        ...
        <a href="{{ route('secretaire.store') }}"
           class="nav-item {{ request()->routeIs('secretaire.store') ? 'active' : '' }}">
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5">
                <circle cx="12" cy="12" r="10"/>
